# ============================================================
# CStar Comparison — SPARSE OBSERVATIONS (obs-x variant)
# UDE_i | GP_i | ARIMA | RandomWalk | LSTM
# State observed: x only — i and c are latent
#
# Sparse-sampling extension of cstar_comparison.jl:
#   FREQ (CLI arg 3, default 3) — only every FREQ-th dense timestep
#   is observed.  e.g. FREQ=3: t=1,4,7,... are observed.
#
# Design:
#   - Full system simulated at Δt=1 (DATASIZE points).
#   - train_size = number of DENSE timesteps in the training window
#     (same meaning as in the dense script — fair comparison).
#   - obs_idx = 1:FREQ:train_size — observed indices within that window.
#   - T_obs = length(obs_idx) = ceil(train_size / FREQ).
#   - X_train_sparse: 2 × T_obs — only observed rows of data_n.
#   - uhat: 3 × T_obs — latent state at observed times only.
#   - L_dyn: bridges FREQ physics steps between consecutive uhat cols.
#   - X_test: HORIZON full-res steps after dense index train_size.
#   - Dip detector: runs on the FULL x trajectory (all DATASIZE points).
#
# CLI: julia script.jl <train_size> <seed> [<freq>] [<consumption>]
#   train_size: number of DENSE timesteps in training window
#   seed:       random seed
#   freq:       observation frequency (default 3)
#   consumption: holling2 | quadratic | linear | constant (default holling2)
#
# Output dirs: comparison_outputs_obs_x_sparse_freq{FREQ}/
# ============================================================
...
# ── CLI args ──────────────────────────────────────────────────────────────────
@assert length(ARGS) >= 2 "Usage: julia script.jl <train_size> <seed> [<freq>] [<consumption>]"
train_size       = parse(Int64, ARGS[1])
seed             = parse(Int64, ARGS[2])
const CONSUMPTION_TYPE = length(ARGS) >= 3 ? Symbol(ARGS[3]) : :holling2
const FREQ             = length(ARGS) >= 4 ? parse(Int64,   ARGS[4]) : 3

Random.seed!(seed)
rng = StableRNG(seed)

base_dir = "/home/ceoas/rathorek/projects/CStar/revision_exp/sampled_op/obs_x_$(CONSUMPTION_TYPE)_$(FREQ)"
plot_dir = "$base_dir/ts$(train_size)_seed$(seed)"
mkpath(plot_dir)

println("="^60)
println("CStar Comparison SPARSE | freq=$FREQ | consumption=$CONSUMPTION_TYPE | Train obs: $train_size | Seed: $seed")
println("="^60)
...
# Switch via CLI arg (ARGS[4]) or hardcode CONSUMPTION_TYPE above.
#   :holling2  →  x²/(x²+h²)   [default; Tilman / type-II]
#   :quadratic →  x²
#   :linear    →  x
#   :constant  →  1             [density-independent harvesting]
consumption(x) =
    CONSUMPTION_TYPE === :holling2  ? x^2 / (x^2 + H_PARAM^2) :
...
function data_prep(train_size, rng)
    ...
    # train_size = dense timesteps in training window (same as dense script)
    @assert train_size <= DATASIZE "train_size=$train_size exceeds DATASIZE=$DATASIZE"
    @assert train_size + HORIZON <= DATASIZE "train_size=$train_size leaves no room for HORIZON=$HORIZON test steps"

    X_train_dense = data_n[:, 1:train_size]
    X_test        = data_n[1, train_size+1:train_size+HORIZON]

    return X_train_dense, X_test, data_n, data
end

X_train_dense, X_test, data_n, data_clean = data_prep(train_size, rng)

# Sparse subsampling within the training window (same logic as obs_xi_sparse)
const obs_idx   = collect(1:FREQ:train_size)
const T_obs     = length(obs_idx)
X_train         = X_train_dense[:, obs_idx]   # 2 × T_obs

x_obs_train = X_train[1, :]   # observed x at sparse times  [T_obs]

# Full-resolution c_real and subsampled version at observed times
c_real      = collect(range(0.0, length=DATASIZE, stop=4.0))
c_real_obs  = c_real[obs_idx]

println("Data ready | dense_train=$train_size | obs_freq=$FREQ | T_obs=$T_obs | obs_idx=$(obs_idx[1]):$(FREQ):$(obs_idx[end]) | test=$(length(X_test))")

# ── Algebraic c on sparse observations ───────────────────────────────────────
# Between observed timesteps t_k and t_{k+1} = t_k + FREQ steps, the
# multi-step x increment is:
#   x[t+FREQ] - x[t] ≈ FREQ * (r*x*(1-x/K) - c*φ(x) + i*x)
# Solving for c (ignoring i, same approximation as dense case):
#   c_alg[k] ≈ (r*xₖ*(1-xₖ/K) + (xₖ - x_{k+1})/FREQ) / φ(xₖ)
# This is the natural sparse extension of the algebraic estimator.
...
# ── Run detector on FULL x trajectory (all DATASIZE points) ──────────────────
# Then subsample regime labels to observed timesteps for classification.
# This is correct: the dip detector needs long windows to work, so it must
# see the full-resolution trajectory even when observations are sparse.
println("\nRunning dip-test detector on full-resolution x trajectory...")
x_full_obs     = Float64.(data_n[1, :])
dip_regime_all, dip_sig, dip_var_thr = run_dip_detector(x_full_obs, c_real)

# Subsample to observed training timesteps
dip_regime_train = dip_regime_all[obs_idx]   # length T_obs

print_dip_summary(dip_regime_all, c_real, obs_idx[end])  # summary up to last obs
...
function algebraic_c(x::AbstractVector{<:Real})
    xₜ = Float64.(x[1:end-1]);  xₜ₁ = Float64.(x[2:end])
    num   = R_PARAM .* xₜ .* (1 .- xₜ./K_PARAM) .+ xₜ .- xₜ₁
    denom = xₜ.^2 ./ (xₜ.^2 .+ H_PARAM^2)
    c_alg = num ./ denom
    c_alg[(xₜ .< 1e-8) .| (abs.(denom) .< 1e-10)] .= NaN
    return c_alg
end
alg_c = max.(algebraic_c(x_obs_train), 0.0)
...
base_res(method, t_secs) = Dict(
    ...
    "freq"=>FREQ, "consumption"=>String(CONSUMPTION_TYPE),
...
function standardize_df(df)
    cols = ["method","seed","train_size","freq","consumption","r","K","h","time",
...
# ── Plot utilities ────────────────────────────────────────────────────────────
function zoomed_forecast_plot(x_obs, X_test, forecasts, obs_idx, tag, out_dir)
    # Show last 15 observed points in training + test horizon
    n_show   = min(15, length(obs_idx))
    show_obs = obs_idx[end-n_show+1:end]      # full-res indices of last n_show obs
    last_full = obs_idx[end]                   # last full-res training index
    T_te_ext  = last_full:last_full+length(X_test)

    p = plot(show_obs, x_obs[end-n_show+1:end],
             label="Observed x (train)", color=:gray, alpha=0.7, lw=2,
             marker=:circle, ms=4,
             xlabel="Time step (full resolution)", ylabel="Abundance (x)",
             title="X Forecast | $tag",
             legend=:best, size=(1100,500), dpi=200; PLOT_MARGINS...)
    plot!(p, T_te_ext, vcat(x_obs[end], X_test),
          label="True x (test)", color=:black, lw=2.5)
    for (lbl, fcast) in forecasts
        plot!(p, T_te_ext, vcat(x_obs[end], fcast),
              label=lbl, color=get(METHOD_COLORS,lbl,"#333333"), lw=2, ls=:dash,
              marker=get(METHOD_MARKERS,lbl,:circle), ms=4)
    end
    vline!(p, [last_full], color=:black, ls=:dot, lw=1.5, label=false)
...
"""
function c_hat_plot(true_c, pred_c_dict, alg_c, dip_regime, obs_idx, tag, out_dir)
    T_obs   = length(obs_idx)
    y_lo = -0.5;  y_hi = 10.0

    p = plot(obs_idx, true_c,
             label="True c", color=:black, lw=2.5,
             xlabel="Time step (full resolution)", ylabel="Net harvesting pressure (c)",
             title="C Recovery + Dip Regimes | $tag",
             legend=:best, size=(1100,500), dpi=200; PLOT_MARGINS...)

    y_bot, y_top = y_lo, y_hi
    for (code, col, alpha) in zip(0:2, REGIME_COLORS, REGIME_ALPHA)
        # dip_regime is already subsampled to obs_idx — map back to full-res x for vspan
        obs_with_code = obs_idx[findall(==(code), dip_regime[1:T_obs])]
        segs = contiguous_segments(collect(obs_with_code))
...
function i_hat_plot(true_i, pred_i_dict, obs_idx, tag, out_dir)
    p = plot(obs_idx, true_i, label="True i (latent)", color=:black, lw=2.0,
             xlabel="Time step (full resolution)", ylabel="Environmental noise (i)",
             title="I Recovery | $tag",
             legend=:best, size=(1100,500), dpi=200; PLOT_MARGINS...)
    for (lbl, pi) in pred_i_dict
        isnothing(pi) && continue
        plot!(p, obs_idx[1:length(pi)], pi,
              label=lbl, color=get(METHOD_COLORS,lbl,"#333333"), lw=2, ls=:dash, alpha=0.85)
    end
...
"""
function regime_overlay_plot(x_full, c_real_full, dip_regime_full, obs_idx, tag, out_dir)
    gr()
    last_obs   = obs_idx[end]
    ts_full    = 1:last_obs

    det_regime_full = [c < CSTAR1 ? 0 : c < CSTAR2 ? 1 : 2
                       for c in c_real_full[1:last_obs]]

    # ── Panel 1: c(t) full res + det shading ─────────────────────────────────
    c_yhi = min(maximum(c_real_full[1:last_obs]) * 1.12, 5.0)
    p1 = plot(ts_full, c_real_full[1:last_obs],
    ...
    for (code, col) in zip(0:2, REGIME_COLORS)
        segs = contiguous_segments(findall(==(code), det_regime_full))
        isempty(segs) && continue
        for (k, (s, e)) in enumerate(segs)
            plot!(p1, Shape([s, e, e, s], [-0.15, -0.15, c_yhi, c_yhi]);
                  color=col, alpha=0.35, lw=0,
                  label=k==1 ? "det: $(REGIME_NAMES[code+1])" : nothing)
        end
    end
    plot!(p1, ts_full, c_real_full[1:last_obs]; color=:black, lw=2.0, label=nothing)
    # Mark observed timesteps on c panel
    scatter!(p1, obs_idx, c_real_full[obs_idx];
             color=:black, ms=3, msw=0, label="observed (freq=$(FREQ))")

    # ── Panel 2: x(t) full res + dip shading + obs markers ───────────────────
    x_yhi = max(maximum(x_full[1:last_obs]) * 1.10, 12.0)
    p2 = plot(ts_full, x_full[1:last_obs],
              color=:gray, lw=1.0, alpha=0.5, label="x (full res)",
              ylabel="x (abundance)", legend=:topright,
              ylim=(-0.5, x_yhi), title="Stochastic regime (dip-test on x)";
              PLOT_MARGINS...)
    for (code, col) in zip(0:2, REGIME_COLORS)
        segs = contiguous_segments(
            findall(i -> i<=last_obs && dip_regime_full[i]==code, 1:last_obs))
        isempty(segs) && continue
        for (k, (s, e)) in enumerate(segs)
            plot!(p2, Shape([s, e, e, s], [-0.5, -0.5, x_yhi, x_yhi]);
                  color=col, alpha=0.35, lw=0,
                  label=k==1 ? "dip: $(REGIME_NAMES[code+1])" : nothing)
        end
    end
    plot!(p2, ts_full, x_full[1:last_obs]; color=:gray, lw=1.0, alpha=0.5, label=nothing)
    scatter!(p2, obs_idx, x_full[obs_idx];
             color=:black, ms=3, msw=0, label="observed (freq=$(FREQ))")

    # ── Panel 3: regime bar comparison det vs dip (at observed times only) ────
    p3 = plot(ylabel="regime source", xlabel="timestep (full resolution)",
              ylim=(-0.6, 1.6), legend=:topright,
              yticks=([0.0, 1.0], ["dip", "det"]),
              title="Regime agreement at observed times: det (top) vs dip (bottom)";
              PLOT_MARGINS...)
    for (code, col) in zip(0:2, REGIME_COLORS)
        for t in findall(==(code), det_regime_full[obs_idx])
            plot!(p3, [obs_idx[t]-0.5, obs_idx[t]+0.5], [1.0, 1.0];
                  color=col, lw=7, label="")
        end
        for t in findall(i -> dip_regime_full[obs_idx[i]]==code, 1:length(obs_idx))
            plot!(p3, [obs_idx[t]-0.5, obs_idx[t]+0.5], [0.0, 0.0];
                  color=col, lw=7, label="")
        end
    end
    ...
    fig = plot(p1, p2, p3;
               ...
               plot_title="Regime overlay (freq=$FREQ) | $tag",
...
function run_ude_i(X_train, X_test, rng, dip_regime_train)
    ...
    # uhat: 3 × T_obs, Float32  (latent state at OBSERVED timesteps only)
    ...
    # Single Δt=1 physics step
    function step1(u, NNc_p, NNi_p)
        x, c, i_val = u[1], u[2], u[3]
        xn     = normalize_x(x)
        Δc     = NN_c(xn, NNc_p, st_c)[1][1]
        ε_i    = NN_i(xn, NNi_p, st_i)[1][1]
        x_next = Float32(clamp(x + R_PARAM*x*(1 - x/K_PARAM) -
                               c*consumption(x) + i_val*x, 1f-8, 1f4))
        c_next = c + Δc
        i_next = Float32(1 - 1/T_PARAM) * i_val + ε_i
        return [x_next, c_next, i_next]
    end

    # Multi-step rollout: FREQ Δt=1 steps between consecutive observed points.
    # Returns the state after FREQ steps AND accumulates intermediate residuals
    # so the dynamical loss penalises the full path, not just endpoints.
    function step_freq(u_start, NNc_p, NNi_p)
        u = u_start
        acc_res = zeros(Float32, 3)   # accumulated squared residuals
        for _ in 1:FREQ
            u_next  = step1(u, NNc_p, NNi_p)
            acc_res = acc_res .+ (u_next .- u).^2   # smoothness of latent path
            u       = u_next
        end
        return u, acc_res
    end
    ...
    function loss(params, data)
        ...
        # L_dyn: at each gap between observed times, roll FREQ steps from
        # uhat[:,k] and compare endpoint to uhat[:,k+1].
        # This enforces that the latent trajectory is physically consistent
        # across the unobserved gap.
        L_dyn = sum(begin
                        u_pred, _ = step_freq(𝐮[:,k], params.NNc, params.NNi)
                        sum((𝐮[:,k+1] .- u_pred).^2)
                    end
                    for k in 1:T-1)
    ...
    true_c  = c_real_obs                        # c at observed timesteps
    true_i  = data_clean[2, obs_idx]            # clean i at observed timesteps
...
function run_gp_i(X_train, X_test, rng, dip_regime_train)
    ...
    function objective(params)
        ...
        i_innov = [𝐢[t] - (1 - 1/T_PARAM)*𝐢[t-1] for t in 2:T_obs]
        lml_i   = -logpdf(construct_gp_i(X_norm, params), i_innov)
        # Multi-step L_dyn: roll FREQ steps between consecutive observed points
        L_dyn   = sum(begin
                          u_pred = [𝐱[k], 𝐜[k], 𝐢[k]]
                          for _ in 1:FREQ
                              u_pred = [physics_step_x_full(u_pred[1], u_pred[2], u_pred[3]),
                                        u_pred[2],   # c held constant over gap (GP learns Δc at obs times)
                                        i_ar1_step(u_pred[3], 0.0)]
                          end
                          (𝐱[k+1] - u_pred[1])^2
                      end
                      for k in 1:T_obs-1)
...
tag      = "train$(train_size)_seed$(seed)_freq$(FREQ)_$(CONSUMPTION_TYPE)"
...
# PLOTS
# ══════════════════════════════════════════════════════════════════════════════
true_i_train = data_clean[2, obs_idx]   # clean latent i at observed timesteps
gr()

# For forecast plot: x_obs on sparse grid, test on full resolution
zoomed_forecast_plot(x_obs_train, X_test,
    ...
    obs_idx, tag, plot_dir)

c_hat_plot(c_real_obs,
    OrderedDict("UDE_i"=>ude_pred_c,"GP_i"=>gp_pred_c),
    alg_c, dip_regime_train, obs_idx, tag, plot_dir)

i_hat_plot(true_i_train,
    OrderedDict("UDE_i"=>ude_pred_i,"GP_i"=>gp_pred_i),
    obs_idx, tag, plot_dir)

# 3-panel regime overlay — full-res x, subsampled dip regime at obs_idx
regime_overlay_plot(Float64.(data_n[1, :]), c_real, dip_regime_all,
                    obs_idx, tag, plot_dir)
